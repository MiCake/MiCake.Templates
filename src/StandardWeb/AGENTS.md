# AGENTS.md — MiCake Project Development Guide
...
|---|---|---|
| Web | `StandardWeb.Web` | Controllers, validators, JWT/CORS host config |
| Application | `StandardWeb.Application` | Use cases: Services, Providers, Mapper, ErrorCodes, Audit |
| EFCore | `StandardWeb.EFCore` | `AppDbContext`, repository implementations |
| Domain | `StandardWeb.Domain` | Aggregate roots, entities, value objects, repository interfaces, domain services |
| Common | `StandardWeb.Common` | Cross-cutting utilities (encryption, time, cache, `OperationResult`) |
| Contracts | `StandardWeb.Contracts` | Pure DTOs and enums |
...
- **On-demand includes**: use `FindAsync(id, includeFunc)` to include navigation properties on demand; avoid fixed full `Include(...)` chains (ref `UserRepo.GetByIdWithIncludesAsync`)
...
- Error code = `{ModuleCode}.{ErrorCode}` (e.g., `02.9900`): module codes in `Web/Constants/ModuleCodes.cs`; error code classes in `Application/ErrorCodes/` extending `BaseErrorCodes`
...
- Keep property setters `private`; create via **static factories** (`User.RegisterWithPhoneNumber(...)`), mutate via **domain methods**; collection navigation properties are read-only
...
**⑥ Migration**: `dotnet ef migrations add AddProduct --project src/StandardWeb.EFCore --startup-project src/StandardWeb.Web`

---

## 5. Authentication

- Login/registration lives in `Application/Services/Auth/AuthService.cs` (`[UnitOfWork]` ensures transactional atomicity); current user info comes from `ICurrentUser` (parsed from JWT claims)
- Protect endpoints with the standard `[Authorize]`
...
- Commands: `dotnet build` / `dotnet test` / `dotnet run --project src/StandardWeb.Web` / `dotnet ef migrations add <Name> --project src/StandardWeb.EFCore --startup-project src/StandardWeb.Web`
