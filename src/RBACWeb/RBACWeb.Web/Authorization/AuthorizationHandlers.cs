...
public class RoleAuthorizationHandler : AuthorizationHandler<RoleRequirement>
{
        ...
        RoleRequirement requirement)
    {
        ...
        var isInRole = await _currentUser.IsInRoleAsync(requirement.RoleCode);
        if (isInRole)
        {
            _logger.LogDebug("User {UserId} has role {Role}", userId, requirement.RoleCode);
            context.Succeed(requirement);
        }
        else
        {
            _logger.LogDebug("User {UserId} does not have role {Role}", userId, requirement.RoleCode);
        }
    }
}
