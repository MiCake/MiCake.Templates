...
This is a **MiCake-based ASP.NET Core 10.0 DDD template** (RBACWeb) providing production-ready scaffolding for web APIs. The template uses the [MiCake framework](https://github.com/MiCake/MiCake) for modular architecture, dependency injection, and Entity Framework Core integration.

## Architecture (Dependency Flow: Web → Application → Domain → Common/Contracts)

| Layer | Project | Responsibility |
|-------|---------|----------------|
| **Web** | `RBACWeb.Web` | Controllers, HTTP concerns, startup, validators |
| **Application** | `RBACWeb.Application` | Services, providers, use-case orchestration |
| **Domain** | `RBACWeb.Domain` | Aggregates, repositories, EF Core DbContext |
| **Infrastructure (EF Core)** | `RBACWeb.EFCore` | Data access implementation: EF Core DbContext, repository implementations |
| **Common** | `RBACWeb.Common` | Cross-cutting helpers (encryption, time, cache) |
| **Contracts** | `RBACWeb.Contracts` | Shared DTOs for cross-service contracts |
...
### Error Codes
- Define module codes in `RBACWeb.Web/Constants/ModuleCodes.cs`
- Define error codes in `RBACWeb.Application/ErrorCodes/` inheriting `BaseErrorCodes`
...
Examples:
- `PagingQueryUserDto` (used only in Web layer) → Place in `RBACWeb.Web/Dtos`.
...
1. Define entities, aggregates, and repository interfaces in `RBACWeb.Domain`;
2. Add repository implementation in `RBACWeb.EFCore`, and register/configure EF Core entities in `AppDbContext`;
3. Implement business logic (use-cases) in `RBACWeb.Application/Services/<Feature>/`, and write AutoMapper Profiles in Application layer;
...
7. Write API controllers and corresponding DTOs/Validators in `RBACWeb.Web/Controllers/<Feature>`;
...
## Test Structure
- Unit tests: `tests/RBACWeb.Domain.Tests/`, `tests/RBACWeb.Application.Tests/`
- Integration tests: `tests/RBACWeb.Web.IntegrationTests/`
- Uses xUnit + Moq
