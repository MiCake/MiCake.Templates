...
RBACWeb 是基于 MiCake 框架构建的一个开箱即用的 ASP.NET Core 10.0 + DDD 模板。它把常见的 Web 服务组成部分（认证、日志、EF Core、映射等）分层封装，便于快速上手并在企业场景中扩展。
...
| 层级 | 项目 | 主要职责 |
| --- | --- | --- |
| Web Host | `RBACWeb.Web` | 应用启动与宿主：Program.cs、Controller、API Host 配置、OpenAPI/Swagger、认证与中间件注册 |
| Application | `RBACWeb.Application` | 应用用例/业务编排：服务（Use-cases）、Providers（如 JwtProvider）、DTO 映射（AutoMapper 配置） |
| Infrastructure（基础设施层） | `RBACWeb.EFCore` | 数据访问实现：EF Core DbContext、仓储实现 |
| Domain | `RBACWeb.Domain` | 领域模型、聚合根、仓储接口（纯领域层，技术无关） |
| Common | `RBACWeb.Common` | 跨层基础设施：工具类（加密、时间等）、共享 contract/类型、认证/Token 辅助、缓存/HttpClient 封装 |
| Contracts | `RBACWeb.Contracts` | 对外 DTO（公共数据契约），供不同层之间（或其他服务）共享使用 |

依赖自上而下流动（Web → Application → EFCore → Domain），领域层保持技术无关性。`RBACWeb.EFCore` 层处理所有数据库相关的实现，而 `RBACWeb.Domain` 包含纯粹的领域逻辑和仓储接口。

## 目录结构（精简视图）
```
src/RBACWeb
├── RBACWeb.Common/       # 跨层基础设施：加解密、时间、Result、Token 辅助等
├── RBACWeb.Contracts/    # 共享 DTO/Contracts（与 Application、Web 对接）
├── RBACWeb.Domain/       # 领域模型、聚合根、仓储接口（纯领域，无基础设施）
├── RBACWeb.EFCore/       # 基础设施层：AppDbContext、仓储实现、EF Core 配置
├── RBACWeb.Application/  # 业务服务、Provider、AutoMapper Profile
├── RBACWeb.Web/          # 启动宿主：Program.cs、控制器、OpenAPI、Startup 扩展
├── tests/                    # 测试项目
...
- 在 `RBACWeb.Web/appsettings.Development.json` 或环境变量中设置：
...
在开发环境运行时，会默认启用 OpenAPI/Scalar 文档（Program.cs 中控制）；当运行`RBACWeb.Web`后，默认会跳转到 `http://localhost:<port>/scalar/v1` 来查看接口和参考文档。

## Contracts（契约层）与 DTO 放置指南

Contracts 层 (`RBACWeb.Contracts`) 的主要用途是放置跨进程/跨项目的共享数据契约（DTOs）和公共接口：
...
- 把 DTO 放到 Web 层的场景：
    ...
    - DTO 与业务逻辑紧密耦合的场景请在 Contracts 层定义（见下文），若仅为 API 层展现和绑定，请放在 `RBACWeb.Web/Dtos`。

示例：
- `PagingQueryUserDto`（仅仅在Web层使用）→ 可放在 `RBACWeb.Web/Dtos`。
- `UserDto`（作为公共用户信息展示且可能被其他服务或客户端复用）→ 放在 `RBACWeb.Contracts/Dtos`。
...
- 放在 Application 层的情景（推荐用于领域 <-> 业务 DTO）：
    - Application 层负责用例与服务编排，常需要将领域模型映射为业务 DTO 或内部传递对象（例如：Domain.User -> Application.UserDto）。将此类 Profile 写在 `RBACWeb.Application/Mapper` 中可提高重用性和测试性。

- 放在 Web 层的情景（推荐用于Web层特有的请求/响应 DTO）：
    - Web 层可能需要针对 HTTP 请求/响应（视图模型、附带字段、API 版本化变化）做特定映射。把这些 Profile 放在 `RBACWeb.Web/Mapper` 能把映射规则与 API 层解耦。
...
## 扩展一个业务模块（建议流程）
1. 在 `RBACWeb.Domain` 中定义实体、聚合及仓储接口等领域对象（保持技术无关）；
2. 在 `RBACWeb.EFCore/Repositories/` 添加仓储实现，并且在 `AppDbContext` 中配置 EF Core 实体映射；
3. 在 `RBACWeb.Application/Services/<模块名>` 实现业务逻辑（Use-cases），并在 Application 层编写 AutoMapper Profile；
4. 在`RBACWeb.Application\ErrorCodes`中定义错误码；（如果需要）
5. 在`RBACWeb.Contracts`中定义跨服务共享的 DTO 和接口；（如果需要）
6. 在 `RBACWeb.Web\Constants\ModuleCodes.cs` 中定义模块码；
7. 在 `RBACWeb.Web/Controllers/<模块名>` 编写 API 控制器和对应 DTO / Validator；
